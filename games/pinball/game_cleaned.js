...
function updateGravityDisplay(beta, gamma, gravityX, gravityY) {
    ...
    const gravityGamma = document.getElementById('gravityGamma');
    const gravityXEl = document.getElementById('gravityX');
    const gravityYEl = document.getElementById('gravityY');
    const gravityBarX = document.getElementById('gravityBarX');
    const gravityBarY = document.getElementById('gravityBarY');

    if (gravityBeta) gravityBeta.textContent = beta.toFixed(1) + '°';
    if (gravityGamma) gravityGamma.textContent = gamma.toFixed(1) + '°';
    if (gravityXEl) gravityXEl.textContent = gravityX.toFixed(3);
    if (gravityYEl) gravityYEl.textContent = gravityY.toFixed(3);
...
function setupEventListeners() {
    ...
    function toggleGravity() {
        ...
        if (gravityEnabled) {
            ...
            if (gravityDisplay) {
                gravityDisplay.classList.add('active');
            }
            enableGravitySensor();
        } else {
            ...
            if (gravityDisplay) {
                gravityDisplay.classList.remove('active');
            }
...
function onMouseDown(e) {
    ...
    const launchRadius = 120;

    const inLaunchZone = distance <= launchRadius;
...
function startChargingLoop() {
    ...
    function charge() {
        if (!isCharging) {
            chargingAnimationId = null;
            return;
        }

        if (power < maxPower) {
            power += chargeRate;
            document.getElementById('powerBar').style.width = (power / maxPower * 100) + '%';
            chargingAnimationId = requestAnimationFrame(charge);
        }
    }
...
function launchBall() {
    ...
    if (chargingAnimationId) {
        cancelAnimationFrame(chargingAnimationId);
        chargingAnimationId = null;
    }
